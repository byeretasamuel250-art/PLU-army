...
async function handleMediaRequest(request){
  ...
  if(!cached){
    ...
    // Pass this first request straight through and let it stream as
    // normal — NOT buffered and length-checked first. An earlier version
    // of this did buffer + verify the whole response before handing it to
    // the player, to catch a connection dropping mid-download without
    // fetch() throwing. That's real, but checking it this way means the
    // player can't receive a single byte until the entire requested chunk
    // has finished downloading — which is exactly what showed up as
    // playback "holding" partway through on a first play, especially for
    // a longer note. Recovering from a genuinely truncated stream is
    // handled client-side instead (see the audio 'error' handler in
    // index.html): if playback does hit a bad spot, it reloads and
    // resumes from where it stopped — usually served instantly from the
    // background download above, which has often finished caching a
    // complete copy by then.
    try{
